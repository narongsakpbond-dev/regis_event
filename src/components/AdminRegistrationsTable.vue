...
<template>
  <div class="rounded-2xl bg-white shadow p-4">
      ...
      <table class="w-full text-sm">
        ...
        <tbody v-if="!loading">
          <tr v-for="r in items" :key="r._id || r.id" class="border-b last:border-0">
            ...
            <td class="py-2 font-mono">{{ r.phone }}</td>
          ...
          <tr v-if="items.length === 0">
            <td colspan="4" class="py-6 text-center text-gray-500">No registrations</td>
          </tr>
        </tbody>

        <tbody v-else>
          <tr>
            <td colspan="4" class="py-6 text-center text-gray-500">Loading...</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="mt-4 flex items-center justify-between text-sm text-gray-600">
      <p>
        Page {{ pagination?.page ?? 1 }} / {{ pagination?.totalPages ?? 1 }} • Total
        {{ pagination?.total ?? 0 }}
      </p>
